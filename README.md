...
Chapter 14
--
In chapter fourteen, we conclude the book by focusing on automating the blackbox web application scanner Arachni, a free and open source project, though dual-licensed. Using both the simpler RESTful HTTP server and the more powerful MSGPACK RPC that is the shipped with the project, we create a small tools to automatically scan a URL and report the findings as we scan.
